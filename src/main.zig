...
fn cast_ray(origin: rl.Vector3, direction: rl.Vector3, objects: []const Forma, lights: []const Light, max_recursion: usize) rl.Vector3 {
    ...
    if (closest_hit) |hit| {
        ...
        for (lights) |light| {
            ...
            // Dirección a la que reflejamos la luz (reflexión de Phong: R = 2(N·L)N - L)
            ...
            // ¿Qué tan intensa y precisa es su reflexión especular?
...
fn obscured(origin: rl.Vector3, light: Light, objects: []const Forma) bool {
    ...
    // Empujamos ligeramente el origen del rayo de sombra en dirección a la
    // luz, para evitar que el rayo choque consigo mismo por errores de
    // precisión de punto flotante ("shadow acne").
    const shadow_origin = origin.add(light_dir.scale(0.001));

    for (objects) |object| {
        if (object.intersect(shadow_origin, light_dir)) |hit| {
            // Solo cuenta si el objeto que bloquea está ENTRE el punto y la
            // luz, no más allá de ella.
